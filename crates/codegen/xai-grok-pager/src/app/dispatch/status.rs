...
pub(super) fn open_usage_info_modal(
    ...
    let is_spacexai = provider == Some(xai_grok_shell::agent::provider_auth::ProviderId::Spacexai);
    let billing_reachable =
        is_spacexai && usage_visible && !agent.chat_kind && redirect_url.is_none();
    ...
    // Populate the account/quota part of the modal from the active provider.
    if billing_reachable {
        ...
        match provider {
            ...
            Some(xai_grok_shell::agent::provider_auth::ProviderId::Openrouter) => {
                state.provider_usage_text =
                    Some("OpenRouter account usage is not available in Forge yet.".to_string());
            }
            None if !agent.chat_kind => {
                state.provider_usage_text = Some(
