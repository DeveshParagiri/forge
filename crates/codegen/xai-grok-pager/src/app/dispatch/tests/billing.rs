...
fn show_usage_routes_codex_by_active_model_provider() {
    ...
    assert!(
        ...
        "session usage must be fetched for the modal: {effects:?}"
    );
    assert!(
        effects.iter().any(|effect| matches!(
            effect,
        ...
        "Codex quota must populate the modal without xAI billing: {effects:?}"
    );
    assert!(
        !effects
            .iter()
            .any(|effect| matches!(effect, Effect::FetchBilling { .. })),
        "Codex usage must not call the xAI billing extension: {effects:?}"
    );
