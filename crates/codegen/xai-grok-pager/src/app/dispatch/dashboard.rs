...
pub(super) fn dispatch_dashboard_stop(app: &mut AppView) -> Vec<Effect> {
    ...
    match &sel {
        ...
        // Forge: imported external sessions are read-only dashboard rows.
        DashboardRowId::External { .. } => vec![],
    }
}
...
fn delete_dashboard_row(
    ...
    match row {
        ...
        // Forge: external transcripts are imported on open and are not local
        // session records that this dashboard may delete.
        DashboardRowId::External { .. } => vec![],
    }
}
