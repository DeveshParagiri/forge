...
pub(super) fn dispatch_task_result(result: TaskResult, app: &mut AppView) -> Vec<Effect> {
    ...
    match result {
        ...
        TaskResult::ProviderUsageFetched {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                let text = crate::forge::provider_usage::format(&snapshot);
                if nonce != 0 {
                    if let Some(state) = usage_modal_state_mut(agent)
                        && state.fetch_nonce == nonce
                    {
                        ...
                        state.provider_usage_text = Some(text);
                    }
                } else {
        ...
        TaskResult::ProviderUsageError {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                let text = format!("{} usage unavailable: {error}", provider.display_name());
                if nonce != 0 {
                    if let Some(state) = usage_modal_state_mut(agent)
                        && state.fetch_nonce == nonce
                    {
                        state.billing_loading = false;
                        state.billing_error = Some(text);
                    }
                } else {
                    agent.scrollback.push_block(RenderBlock::system(text));
                }
