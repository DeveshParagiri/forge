...
pub struct UsageInfoContext {
    ...
    /// Provider that owns the account/quota surface.
    pub provider: Option<xai_grok_shell::agent::provider_auth::ProviderId>,
...
pub struct UsageInfoModalState {
    ...
    /// Pre-formatted provider account/quota summary for non-SpaceXAI sessions.
...
fn usage_limit_lines(
    ...
    if let Some(text) = &state.provider_usage_text {
    ...
    } else if state.ctx.provider
        == Some(xai_grok_shell::agent::provider_auth::ProviderId::OpenaiCodex)
        && state.billing_loading
    {
        lines.push(muted_line(theme, "Loading OpenAI Codex usage\u{2026}"));
    } else if state.ctx.provider
        == Some(xai_grok_shell::agent::provider_auth::ProviderId::OpenaiCodex)
    {
        let message = state
            .billing_error
            .as_deref()
            .unwrap_or("OpenAI Codex usage is unavailable.");
        lines.push(muted_line(theme, message.to_string()));
