...
impl SamplingClient {
    ...
    /// seed key cannot ride on the wire. Backend-specific filtering
    /// happens after tracing injection; xAI-private headers are stripped
    /// for non-xAI endpoints.
    fn post(&self, url: impl reqwest::IntoUrl) -> (reqwest::RequestBuilder, Option<String>) {
        ...
        // Forge: never forward xAI-private headers to non-xAI endpoints.
    ...
    fn post_with_grok_headers(
        ...
        let (builder, sent_bearer) = self.post(url);
        let builder = if self.accepts_xai_private_headers() {
    ...
    pub async fn chat_completion(
        ...
        let (builder, sent_bearer) =
            self.post_with_grok_headers(self.endpoint("chat/completions"), &grok_headers);
    ...
    pub async fn create_response(
        ...
        let (builder, sent_bearer) =
            self.post_with_grok_headers(self.endpoint("responses"), &grok_headers);
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request =
            builder.header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn create_message(
        ...
        let (builder, sent_bearer) =
            self.post_with_grok_headers(self.endpoint("messages"), &grok_headers);
        let http_request = builder.json(&request.inner);
    ...
    fn all_third_party_request_shapes_strip_xai_headers_after_injection() {
        ...
        for base_url in [
            ...
            for path in ["chat/completions", "responses", "messages"] {
                let (builder, _sent) = client.post(client.endpoint(path));
                let request = builder.build().expect("build");
            ...
            let (builder, _sent) =
                client.post_with_grok_headers(client.endpoint("responses"), &grok_headers);
            let request = builder.build().expect("build");
            assert!(
                request
                    .headers()
