...
impl SamplingClient {
    ...
    /// Resolve headers shared by all request shapes. When a bearer resolver is
    /// wired it is the sole auth source: a missing live bearer strips default
    /// Authorization / x-api-key so an expired seed key cannot ride the wire.
    /// Backend-specific filtering happens after tracing injection.
