...
mod tests {
    ...
    fn completed_response(evs: &[SamplingEvent]) -> &ConversationResponse {
    ...
    async fn function_call_emits_initial_id_name_then_arg_deltas() {
        ...
        let response = completed_response(&evs);
        let assistant = response.assistant().expect("terminal assistant");
    ...
    #[tokio::test]
    async fn completed_function_call_item_recovers_when_added_event_is_missing() {
        let events: Vec<Result<rs::ResponseStreamEvent, SamplingError>> = vec![
