...
fn local_grok_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("forge{}", std::env::consts::EXE_SUFFIX))
}
...
fn ensure_local_grok_binary(binary: &Path) {
    ...
    cmd.current_dir(workspace_root())
        .args(["build", "-p", "xai-grok-pager-bin", "--bin", "forge"])
        .stdin(std::process::Stdio::null())
        .envs(xai_tty_utils::pager_env());
    xai_tty_utils::detach_std_command(&mut cmd);
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build forge: {e}"));

    assert!(
        output.status.success(),
        "failed to build forge for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
...
/// Resolve the Forge binary from `GROK_BINARY` (CI) or a local build.
