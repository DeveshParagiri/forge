...
const EXTERNAL_SUBAGENTS: [(&str, &str); 2] = [
    (
        "claude-code",
        "External Claude Code CLI running headlessly with structured streaming. Use for an independent Claude Code implementation or review perspective; it uses CLI-native tools and authentication rather than Forge's hosted tools.",
    ),
    (
        "codex-cli",
        "External Codex CLI running headlessly with structured streaming. Use for an independent Codex implementation or review perspective; it uses CLI-native tools and authentication rather than Forge's hosted tools.",
    ),
];

/// Add Forge's external harness adapters to the discoverable task roster.
pub(crate) fn append(
