...
pub struct Config {
    ...
    /// Warnings from `[model.*]`, `[auth_provider.*]`, and
    /// `[model_providers.*]` parsing; surfaced by `grok inspect`.
    #[serde(skip)]
    ...
    /// Forge: user-defined shortcut overrides are consumed by the pager.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
///
/// Adopts upstream auth-provider token orchestration while preserving Forge's
/// fail-closed endpoint policy: neither the SpaceXAI session token nor ambient
/// `XAI_API_KEY` is handed to a third-party endpoint. A declared
/// `auth_provider` with no cached token also fails closed rather than falling
/// through to first-party credentials.
pub(crate) fn resolve_credentials(
    model: &ModelEntry,
    session_key: Option<&str>,
) -> ResolvedCredentials {
    let info = model.info();
    // First-party credentials are attached only to positively recognized xAI
    // HTTPS endpoints. Unknown/custom hosts fail closed instead of relying on a
    // denylist of known third parties.
    let accepts_first_party_bearer = crate::util::is_xai_api_bearer_url(&info.base_url);
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        ...
        // Fail closed on a helper miss: `cached_token()` returns `None` when the
        // mint failed or is stale, and we deliberately do NOT fall through to
        // the session/global key for a model that declared an auth_provider.
        (
            provider.cached_token(),
            info.base_url.clone(),
            xai_chat_state::AuthType::ApiKey,
        )
    } else if let Some(key) = session_key.filter(|_| accepts_first_party_bearer) {
        (
            Some(key.to_owned()),
            info.base_url.clone(),
            xai_chat_state::AuthType::SessionToken,
        )
    } else if accepts_first_party_bearer
...
mod tests {
    ...
    #[test]
    fn shortcuts_config_is_absorbed_without_unknown_key_warning() {
        let raw: toml::Value = toml::from_str(
            r#"
[shortcuts]
cycle_effort = "shift+tab"
"#,
        )
        .unwrap();
        ...
        assert!(config.config_warnings.iter().all(|warning| {
            !matches!(
                &warning.target,
                super::super::config_model_override_parse::WarningTarget::ConfigKey { path }
                    if path == "shortcuts"
            )
        }));
    }
