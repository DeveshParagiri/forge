...
/// pre-turn mint via [`crate::auth::AuthProviderRef`]) while preserving Forge's
/// fail-closed behavior for third-party model bases: neither the SpaceXAI
/// session token nor the ambient `XAI_API_KEY` are ever handed to a
/// third-party endpoint. A model with an explicit `auth_provider` and no
/// minted token fails closed (no key) rather than leaking first-party creds.
