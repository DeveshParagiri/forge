        ...
        tokio::select! {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        // Forge: Fast Mode is session-scoped sampling state, not a model switch.
                        SessionCommand::GetSamplingFastMode { responds_to } => {
                            let enabled = session
                                .chat_state_handle
                                .get_sampling_config()
                                .await
                                .and_then(|config| config.fast_mode)
                                .unwrap_or(false);
                            let _ = responds_to.send(enabled);
                        }
                        SessionCommand::SetSamplingFastMode { enabled, responds_to } => {
                            let result = session.handle_set_sampling_fast_mode(enabled).await;
