...
export interface ThreadComposerProps {
  ...
  /** Forge Remote hides controls that its protocol cannot execute. */
  readonly remoteOnly?: boolean;
  /** Forge Remote only exposes Stop when the selected snapshot supports cancellation. */
  readonly remoteCancellable?: boolean;
  /** Prevents duplicate model commands while the gateway acknowledgement is pending. */
...
export const ThreadComposer = memo(function ThreadComposer(props: ThreadComposerProps) {
  ...
  const [remoteEditorHeight, setRemoteEditorHeight] = useState(REMOTE_COMPOSER_EDITOR_MIN_HEIGHT);
  ...
  const invalidBareBtw = props.remoteOnly && /^\/btw\s*$/.test(props.draftMessage.trim());
  const canSend = hasContent && !invalidBareBtw;
  ...
  const handleRemoteContentSizeChange = useCallback(
    (size: { readonly width: number; readonly height: number }) => {
      const nextHeight = resolveRemoteComposerEditorHeight(size.height);
  ...
  const showStopAction = props.remoteOnly
    ? forgeComposerAction(props.draftMessage, running && props.remoteCancellable === true) ===
      "stop"
    : running;
  ...
  const connectionStatus = props.remoteOnly
    ? null
  ...
  const pathSearch: ComposerPathSearchState = { entries: [], isPending: false, error: null };
  ...
  const composerMenuItems: ComposerCommandItem[] = useMemo(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      ...
      const allBuiltIn = props.remoteOnly
        ? [
            {
              id: "cmd:btw",
              ...
              command: "btw",
              label: "/btw",
              description: "Ask without interrupting the current turn",
            },
              ...
              ? [
                  {
                    ...
                    command: "usage",
                    label: "/usage",
                    description: "Open context, session, and account usage",
                  },
                ]
              : []),
          ]
  ...
  const handleSend = useCallback(async () => {
    if (showStopAction && props.remoteOnly) {
      props.onStopThread();
      return;
    }
  ...
  const handleSettingsMenuAction = useCallback(
    (eventId: string) => {
      ...
      switch (event.type) {
        case "select-model":
          if (remoteModelSelectionDisabled) return;
        ...
        >
          ...
          >
            <ComposerEditor
              ...
              onBlur={handleBlur}
              onSubmit={handleSend}
              onContentSizeChange={props.remoteOnly ? handleRemoteContentSizeChange : undefined}
              scrollEnabled={
                isExpanded &&
                (!props.remoteOnly || remoteEditorHeight >= REMOTE_COMPOSER_EDITOR_MAX_HEIGHT)
              }
          ...
          <Animated.View entering={FadeIn.duration(160)} exiting={FadeOut.duration(120)}>
            <ComposerToolbarRow paddingBottom={8} paddingHorizontal={0} paddingTop={8}>
              ...
              >
                ...
                {showStopAction && !props.remoteOnly ? (
                  <ComposerToolbarButton
