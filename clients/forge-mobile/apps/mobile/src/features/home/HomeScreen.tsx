...
function deriveEmptyState(props: {
  ...
  if (props.remoteOnly) {
    return remoteHomeEmptyState(catalogState.hasConnections);
  }
...
export function HomeScreen(props: HomeScreenProps) {
  ...
  const iosBottomToolbarClearance =
    Platform.OS === "ios" && !NATIVE_LIQUID_GLASS_SUPPORTED
      ...
      : 0;
  const threadSearch = {
    matches: [] as ReadonlyArray<EnvironmentThreadSearchMatch>,
    isPending: false,
  };
  ...
  const effectiveGroupDisplayStates = groupDisplayStates;
  ...
  const serverConfigs = useMemo<ReadonlyMap<EnvironmentId, ServerConfig>>(() => new Map(), []);
      ...
      >
        <View className="w-full max-w-[430px]">
          <EmptyState
            ...
            actionLabel={!props.catalogState.hasReadyEnvironment ? "Pair session" : undefined}
