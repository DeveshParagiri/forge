...
describe("pending user input answers", () => {
  ...
  it("clears selected options while a custom answer is active", () => {
    ...
    ).toEqual({ customAnswer: "", selectedOptionLabels: ["Go"] });
  });

  it("does not offer a custom answer when the remote question forbids freeform", () => {
    expect(allowsPendingUserInputCustomAnswer({ ...singleSelectQuestion, allowCustomAnswer: false })).toBe(false);
    expect(allowsPendingUserInputCustomAnswer(singleSelectQuestion)).toBe(true);
  });
