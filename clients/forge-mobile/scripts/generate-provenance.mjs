...
const ROOT_FILES = ["package.json", "pnpm-lock.yaml", "pnpm-workspace.yaml", "tsconfig.base.json", "vite.config.ts"];
...
const EXCLUDED_UPSTREAM_FILES = [
  {
    ...
    reason: "Unused Clerk presentation theme excluded from the Forge deliverable; the active app has no Clerk integration.",
  },
];
...
if (revision !== PINNED_REVISION) throw new Error(`Expected ${PINNED_REVISION}, received ${revision}`);
...
const copiedMappings = [
  ...
  ...ASSET_ROOTS.flatMap((relativeRoot) =>
    filesBelow(path.join(UPSTREAM, relativeRoot)).map((file) => {
      ...
      return { upstreamPath, currentPath: `third_party/t3-assets/${upstreamPath.slice("assets/".length)}` };
    }),
  ),
]
...
const copiedFiles = copiedMappings.map(({ upstreamPath, currentPath }) => {
  ...
  return {
    ...
    upstreamSha256,
    currentSha256,
    status: currentSha256 === null ? "missing" : currentSha256 === upstreamSha256 ? "identical" : "adapted",
  };
});
...
for (const entry of adapted) {
  const result = spawnSync(
    "git",
    ["diff", "--no-index", "--no-ext-diff", "--no-color", "--unified=3", "--", path.join(UPSTREAM, entry.upstreamPath), path.join(ROOT, entry.currentPath)],
    { encoding: "utf8", maxBuffer: 128 * 1024 * 1024 },
  );
  if (result.status !== 0 && result.status !== 1) throw new Error(result.stderr || `Could not diff ${entry.currentPath}`);
...
const manifest = {
  schemaVersion: 1,
  upstream: { repository: "https://github.com/pingdotgg/t3code.git", revision: PINNED_REVISION, license: "MIT" },
...
fs.writeFileSync(path.join(provenanceDir, "t3-mobile-files.json"), `${JSON.stringify(manifest, null, 2)}\n`);
...
const brandMappings = [
  ...
  ["src/assets/fonts/basiersquaremono-regular-webfont.woff2", "brand/basiersquaremono-regular-webfont.woff2", "source"],
  ["public/apple-icon.png", "brand/apple-icon.png", "source"],
  ["public/icon.svg", "brand/icon.svg", "source"],
  ["public/logo.png", "brand/logo.png", "source"],
  ...
  [null, "apps/mobile/assets/forge/icon.png", "1024x1024-derived-from-brand/apple-icon.png"],
...
process.stdout.write(`Generated ${copiedFiles.length} copied mappings: ${manifest.identicalFileCount} identical, ${manifest.adaptedFileCount} adapted, ${manifest.missingFileCount} missing.\n`);
